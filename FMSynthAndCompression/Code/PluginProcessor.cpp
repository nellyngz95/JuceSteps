...
void FMSynthCompressionAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    //Compressor values.
    float threshold=parameters.getRawParameterValue("Threshold")->load();
    float ratio=parameters.getRawParameterValue("Ratio")->load();
    ...
    float makeUpGain=parameters.getRawParameterValue("MakeUpGain")->load();
    ...
    float sampleRate = (float) juce::AudioProcessor::getSampleRate();
     ...
     {
        modulatorSignal = modulationDepth * std::sin(juce::MathConstants<float>::twoPi * modulatorPhase);
        modulatedCarrierSignal = std::sin(juce::MathConstants<float>::twoPi * modulatedCarrierPhase);
...
juce::AudioProcessorValueTreeState::ParameterLayout FMSynthCompressionAudioProcessor::createParameterLayout()
{
    ...
    //Sliders compression
    parameters.push_back(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"Threshold", 1},"Threshold", -60.0f, 0.0f, 0.0f));
    parameters.push_back(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"Ratio", 1},"Ratio", 1.0f, 20.0, 1.0f));
    ...
    parameters.push_back(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"MakeUpGain", 1},"MakeUpGain",  0.0f, 24.0, 0.0f));
