...
void FiltersLabAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    lowFilters.clear();
     ...
     //Clear highFilters and create the for loop for the highFilters.
}
...
void FiltersLabAudioProcessor::loadFile(const juce::File& file)
{
    ...
    if (reader != nullptr) //if the reader is not empty
    {
        ...
        auto newSource = std::make_unique<juce::AudioFormatReaderSource>(reader, true); //Starting variable who is gonna store the content.
...
void FiltersLabAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    // Get parameters from the play and stop button.

    //Losad the parameters from your filters. Frequency, Gain and Q.(High and Low filters)
    ...
    if (play->load() > 0.5f)
    {
        ...
        {
            // Generate a signal if we do not have a file loaded.
            for (int channel = 0; channel < numInputChannels; ++channel)
            {
                float* channelData = buffer.getWritePointer(channel);
                for (int i = 0; i < numSamples; ++i)
                    channelData[i] = //Create your signal
            }
        }
    }
    else
   ...
   //Apply the high filters.

    for (int channel = 0; channel < numInputChannels; ++channel)
    {
        float* channelData = buffer.getWritePointer(channel);
        lowFilters[channel]->processSamples(channelData, numSamples);
       //Process the high Filter in [channel]
    }
}
//==============================================================================
