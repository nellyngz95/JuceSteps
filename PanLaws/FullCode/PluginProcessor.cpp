...
void PanningAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    float pan = panning->load();
    float dist = distance->load();
    float distanceGain   = 1.0f / (1.0f + dist);

    float leftpan  = std::cos(pan * (juce::MathConstants<float>::pi / 2.0f));
    float rightpan = std::sin(pan * (juce::MathConstants<float>::pi / 2.0f));
...
juce::AudioProcessorValueTreeState::ParameterLayout PanningAudioProcessor::createParameterLayout()
{
    ...
    parameters.push_back(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{"panning",  1}, "Panning",  0.0f, 1.0f, 0.5f));
